...
function CargoRepositoryMongo(collection, findLocation, findVoyage, lookupHandlingHistoryOfCargo) {
  ...
  async function store(_, trackingId, originCode, routeSpecOriginCode, routeSpecDestCode, arrivalDeadline, legs) {
    const doc = CargoMapper.toDocument(trackingId, originCode, routeSpecOriginCode, routeSpecDestCode, arrivalDeadline, legs);
